...
zonasi = [p for p in pendaftar if p.jalur == "Zonasi"]
prestasi = [p for p in pendaftar if p.jalur == "Prestasi"]
afirmasi = [p for p in pendaftar if p.jalur == "Afirmasi"]

# Sort data setiap jalur
zonasi.sort(key=sort_zonasi)
prestasi.sort(key=sort_prestasi)
...
diterima.extend(zonasi[:kuota_zonasi])
diterima.extend(prestasi[:kuota_prestasi])
diterima.extend(afirmasi[:kuota_afirmasi])
